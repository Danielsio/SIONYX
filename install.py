...
def collect_firebase_config():
    ...
    if env_path.exists():
        ...
        if existing_config.get('FIREBASE_API_KEY'):
            reuse = input(f"{Colors.BOLD}Use existing Firebase configuration? (y/n){Colors.ENDC} [y]: ").strip().lower()
            if reuse in ['', 'y', 'yes']:
    ...
    config['FIREBASE_AUTH_DOMAIN'] = prompt_input("Firebase Auth Domain (e.g., your-project.firebaseapp.com)")
    config['FIREBASE_DATABASE_URL'] = prompt_input("Firebase Database URL (e.g., https://your-project.firebaseio.com)")
...
def collect_payment_config():
    ...
    has_nedarim = input(f"{Colors.BOLD}Configure Nedarim now? (y/n){Colors.ENDC} [y]: ").strip().lower()
    ...
    if has_nedarim in ['', 'y', 'yes']:
        ...
        config['NEDARIM_API_VALID'] = prompt_input("Nedarim API Valid Key", required=False, secure=True)
        config['NEDARIM_CALLBACK_URL'] = prompt_input("Nedarim Callback URL (Firebase Function URL)", required=False)
        
        if not config['NEDARIM_MOSAD_ID']:
            print_warning("Payment features will be disabled until you add Nedarim credentials")
    else:
...
def collect_organization_info():
    ...
    print()
    print("The Organization ID uniquely identifies this organization in the shared database.")
    ...
    while True:
        org_id = prompt_input("Organization ID (lowercase, letters/numbers/hyphens)").lower()
    ...
    org_name = prompt_input("Organization Display Name", default=org_id.replace('-', ' ').title())
...
def collect_admin_info():
    ...
    print_info("This user will be able to manage packages and view users via the admin dashboard")
    print()
...
def confirm_installation(firebase_config, payment_config, org_info, admin_info):
    ...
    if payment_config.get('NEDARIM_MOSAD_ID'):
        ...
        print(f"  Callback URL: {payment_config.get('NEDARIM_CALLBACK_URL', 'Not set')}")
    else:
    ...
    confirm = input(f"{Colors.BOLD}Proceed with installation? (yes/no){Colors.ENDC}: ").strip().lower()
...
"""
    ...
    if env_path.exists():
        ...
        print_warning(f".env file already exists! Creating backup at {backup_path.name}")
        import shutil
...
def create_organization_and_admin(firebase_config, org_info, admin_info):
    ...
    try:
        ...
        print_success(f"Admin user created: {admin_info['first_name']} {admin_info['last_name']}")
        print()
...
def print_final_instructions(org_info, admin_info, env_path):
    ...
    print(f"{Colors.GREEN}{Colors.BOLD}Organization '{org_info['org_name']}' is ready!{Colors.ENDC}\n")
    
    print(f"{Colors.BOLD}Next Steps:{Colors.ENDC}")
    print(f"  1. Install dependencies: {Colors.CYAN}pip install -r requirements.txt{Colors.ENDC}")
    ...
    print(f"  3. Update config: {Colors.CYAN}src/config/firebase.js{Colors.ENDC} (use same Firebase credentials)")
...
def main():
    ...
    try:
        ...
        print(f"{Colors.BOLD}This wizard will set up a new organization in your SIONYX system{Colors.ENDC}\n")
        ...
        print("  • You can run this installer multiple times for different organizations")
        print()
        ...
        ready = input(f"{Colors.BOLD}Ready to begin? (yes/no){Colors.ENDC}: ").strip().lower()
        ...
        # Confirm
        if not confirm_installation(firebase_config, payment_config, org_info, admin_info):
            print_info("Installation cancelled")
            return
        ...
        if success:
            ...
            print_info("You may need to run the installation again or manually fix the issue.")
