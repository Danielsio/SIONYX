...
class TestSetPurchaseData:
    ...
    def test_sets_purchase_service(self, payment_bridge, mock_purchase_service):
        ...
        payment_bridge.set_purchase_data(
            mock_purchase_service, "user-123", {"id": "pkg-1"}
        )
...
class TestCreatePendingPurchase:
    ...
    def test_calls_create_pending_purchase(self, payment_bridge, mock_purchase_service):
        ...
        mock_purchase_service.create_pending_purchase.assert_called_once_with(
            "user-123", package
        )

    def test_returns_success_on_purchase_created(
        self, payment_bridge, mock_purchase_service
    ):
    ...
    def test_returns_error_on_exception(self, payment_bridge, mock_purchase_service):
        ...
        mock_purchase_service.create_pending_purchase.side_effect = Exception(
            "Database error"
        )
        payment_bridge.set_purchase_data(
            mock_purchase_service, "user-123", {"id": "pkg-1"}
    ...
    def test_does_not_emit_signal_on_failure(
        self, payment_bridge, mock_purchase_service
    ):
        """Test does not emit signal when purchase creation fails"""
        mock_purchase_service.create_pending_purchase.return_value = {
            "success": False,
            "error": "Failed",
        }
        payment_bridge.set_purchase_data(
...
class TestPaymentSuccess:
    ...
    def test_emits_payment_success_signal(self, payment_bridge):
        ...
        signal_received = []
        payment_bridge.payment_success.connect(
