...
/**
 * Verify caller is a supervisor and return their profile.
 * @param {string} uid - Firebase Auth UID
 * @param {object} log - Logger instance
 * @return {object} Supervisor profile data
 */
const verifySupervisor = async (uid, log) => {
  const supervisorRef = admin.database().ref(`supervisors/${uid}`);
  const snapshot = await supervisorRef.once("value");
  if (!snapshot.exists()) {
    log.warn("Caller is not a supervisor", {uid});
    throw new functions.https.HttpsError(
        "permission-denied", "Not a supervisor",
    );
  }
  return snapshot.val();
};
...
/**
 * Get the list of org IDs supervised by a supervisor.
 * @param {string} uid - Supervisor UID
 * @return {string[]} Array of org IDs
 */
const getSupervisedOrgIds = async (uid) => {
  const orgsRef = admin.database()
      .ref(`supervisors/${uid}/organizations`);
...
/**
 * Block a user by phone across all supervised organizations.
 * Writes to supervisors/{uid}/blockedUsers and sets blocked=true
 * on matching users in all supervised orgs.
 */
exports.blockUser = onCall(async (request) => {
  const correlationId = generateCorrelationId();
  const log = createLogger({correlationId, service: "block-user"});
  ...
  const callerUid = request.auth.uid;
  await verifySupervisor(callerUid, log);

  const {phone, reason, userName} = request.data || {};
  if (!phone) {
    throw new functions.https.HttpsError(
        "invalid-argument", "Phone number is required",
    );
  }
  ...
  if (!normalizedPhone) {
    throw new functions.https.HttpsError(
        "invalid-argument", "Invalid phone number",
    );
  }

  log.info("Blocking user", {normalizedPhone, reason, callerUid});

  const orgIds = await getSupervisedOrgIds(callerUid);
  if (orgIds.length === 0) {
    throw new functions.https.HttpsError(
        "failed-precondition", "No supervised organizations",
    );
  }

  // Write to supervisor's blocked list
  const blockedRef = admin.database()
      .ref(`supervisors/${callerUid}/blockedUsers/${normalizedPhone}`);
  ...
  // Block user in all supervised orgs
  let blockedCount = 0;
  for (const orgId of orgIds) {
    ...
    for (const [userId, userData] of Object.entries(users)) {
      ...
      if (userPhone === normalizedPhone) {
        ...
        blockedCount++;
        log.info("User blocked in org", {orgId, userId});
      }
    }
  }

  log.info("Block operation complete", {
    normalizedPhone, blockedCount, orgCount: orgIds.length,
  });

  return {
    success: true,
    blockedCount,
    message: `User blocked in ${blockedCount} organization(s)`,
    correlationId,
  };
});

/**
 * Unblock a user by phone across all supervised organizations.
 * Removes from supervisors/{uid}/blockedUsers and clears blocked flag
 * on matching users in all supervised orgs.
 */
exports.unblockUser = onCall(async (request) => {
  const correlationId = generateCorrelationId();
  const log = createLogger({correlationId, service: "unblock-user"});
  ...
  const normalizedPhone = phone.replace(/\D/g, "");
  log.info("Unblocking user", {normalizedPhone, callerUid});

  const orgIds = await getSupervisedOrgIds(callerUid);

  // Remove from supervisor's blocked list
  await admin.database()
      .ref(`supervisors/${callerUid}/blockedUsers/${normalizedPhone}`)
      .remove();

  // Check if any OTHER supervisor still blocks this phone for each org
  // (multi-supervisor scenario)
  const supervisorsSnap = await admin.database()
      .ref("supervisors").once("value");
  const allSupervisors = supervisorsSnap.exists() ?
      supervisorsSnap.val() : {};

  // Unblock in all supervised orgs (unless another supervisor blocks them)
  let unblockedCount = 0;
  for (const orgId of orgIds) {
    // Check if another supervisor still blocks this phone for this org
    let stillBlocked = false;
    for (const [supId, supData] of Object.entries(allSupervisors)) {
      if (supId === callerUid) continue;
      const supOrgs = supData.organizations || {};
      const supBlocked = supData.blockedUsers || {};
      if (supOrgs[orgId] && supBlocked[normalizedPhone]) {
        stillBlocked = true;
        break;
      }
    }
    ...
    if (stillBlocked) {
      log.info("User still blocked by another supervisor", {
    ...
    for (const [userId, userData] of Object.entries(users)) {
      ...
      if (userPhone === normalizedPhone) {
        ...
        unblockedCount++;
        log.info("User unblocked in org", {orgId, userId});
      }
    }
  }

  log.info("Unblock operation complete", {
    normalizedPhone, unblockedCount,
  });

  return {
    success: true,
    unblockedCount,
    message: `User unblocked in ${unblockedCount} organization(s)`,
    correlationId,
  };
});

/**
 * Get metadata and stats for all organizations supervised by the caller.
 */
exports.getSupervisorOrgs = onCall(async (request) => {
  const correlationId = generateCorrelationId();
  const log = createLogger({correlationId, service: "get-supervisor-orgs"});
  ...
  const orgIds = await getSupervisedOrgIds(callerUid);
  log.info("Fetching supervised orgs", {count: orgIds.length});
  ...
  for (const orgId of orgIds) {
    ...
    const activeUsers = Object.values(users)
        .filter((u) => u.isSessionActive).length;
  ...
  const blockedRef = admin.database()
      .ref(`supervisors/${callerUid}/blockedUsers`);
  const blockedSnap = await blockedRef.once("value");
  const blockedCount = blockedSnap.exists() ?
      Object.keys(blockedSnap.val()).length : 0;

  log.info("Supervisor orgs fetched", {
    orgCount: orgs.length, blockedCount,
  });
...
/**
 * Update supervisor-controlled settings for a specific organization.
 * Currently supports operating hours.
 */
exports.setSupervisorOrgSettings = onCall(async (request) => {
  const correlationId = generateCorrelationId();
  const log = createLogger({
    correlationId, service: "set-supervisor-org-settings",
  });
  ...
  if (!orgId || !settings) {
    throw new functions.https.HttpsError(
        "invalid-argument", "Missing orgId or settings",
    );
  }

  // Verify this org is supervised by the caller
  const orgIds = await getSupervisedOrgIds(callerUid);
  if (!orgIds.includes(orgId)) {
    throw new functions.https.HttpsError(
        "permission-denied",
        "Organization is not under your supervision",
    );
  }

  log.info("Updating supervisor org settings", {
    orgId, settingsKeys: Object.keys(settings),
  });

  // Only allow specific settings to be updated by supervisor
  const allowedKeys = ["operatingHours"];
  ...
  if (Object.keys(filteredSettings).length === 0) {
    throw new functions.https.HttpsError(
        "invalid-argument", "No valid settings provided",
    );
  }

  const settingsRef = admin.database()
      .ref(`organizations/${orgId}/metadata/settings`);
  await settingsRef.update(filteredSettings);

  log.info("Supervisor org settings updated", {orgId, filteredSettings});
