...
class TestPurchaseService:
    ...
    def purchase_service(self, mock_firebase_client):
        ...
        with patch(
            "services.purchase_service.DatabaseService.__init__", return_value=None
        ):
    ...
    def test_create_pending_purchase_success(
        self, purchase_service, mock_firebase_client, sample_package
    ):
        ...
        with patch(
            "services.purchase_service.requests.post", return_value=mock_response
        ):
            result = purchase_service.create_pending_purchase(
                "user-123", sample_package
            )

        assert result["success"] is True
        assert result["purchase_id"] == "purchase-id-123"

    def test_create_pending_purchase_firebase_failure(
        self, purchase_service, mock_firebase_client, sample_package
    ):
    ...
    def test_create_pending_purchase_exception(
        self, purchase_service, mock_firebase_client, sample_package
    ):
        """Test creating purchase with exception"""
        with patch(
            "services.purchase_service.requests.post",
    ...
    def test_create_pending_purchase_data_structure(
        self, purchase_service, mock_firebase_client, sample_package
    ):
        ...
        with patch(
            "src.services.purchase_service.requests.post", return_value=mock_response
        ) as mock_post:
    ...
    def test_get_purchase_status_success(self, purchase_service, mock_firebase_client):
        ...
        purchase_data = {"status": "completed", "amount": 29.99, "userId": "user-123"}
    ...
    def test_get_purchase_status_not_found(
        self, purchase_service, mock_firebase_client
    ):
    ...
    def test_get_user_purchase_history_success(self, purchase_service):
        ...
        with patch.object(
            purchase_service,
            "get_all_documents",
            return_value={"success": True, "data": all_purchases},
        ):
            with patch.object(purchase_service, "log_operation"):
                with patch.object(
                    purchase_service, "create_success_response"
                ) as mock_response:
    ...
    def test_get_user_purchase_history_empty(self, purchase_service):
        ...
        all_purchases = [{"userId": "other-user", "amount": 10}]
    ...
    def test_get_purchase_statistics_success(self, purchase_service):
        ...
        with patch.object(
            ...
            return_value={"success": True, "data": user_purchases},
        ):
            with patch.object(purchase_service, "log_operation"):
                with patch.object(
                    purchase_service,
                    "safe_int",
                    side_effect=lambda x: int(x) if x else 0,
                ):
                    with patch.object(
                        purchase_service, "create_success_response"
                    ) as mock_response:
