...
class MessageDisplay(QWidget):
    ...
    def refresh_display(self):
        ...
        if not self.messages:
            ...
            unread_count = sum(1 for msg in self.messages if not msg.get('isRead', False))
            self.count_label.setText(str(unread_count) if unread_count > 0 else str(len(self.messages)))
