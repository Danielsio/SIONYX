...
class PrintDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.job_details_label.setStyleSheet(f"font-weight: bold; color: {Colors.TEXT_PRIMARY};")
        ...
        self.budget_label.setStyleSheet(f"font-weight: bold; color: {Colors.TEXT_PRIMARY};")
        
        self.current_budget_label = QLabel("יתרה נוכחית: 0₪")
        self.remaining_after_label = QLabel("יתרה לאחר הדפסה: 0₪")
        self.status_label = QLabel("")
        
        for label in [self.current_budget_label, self.remaining_after_label, self.status_label]:
            label.setStyleSheet(f"color: {Colors.TEXT_SECONDARY}; margin-left: 10px;")
    ...
    def setup_connections(self):
        ...
        self.print_validation_service.print_budget_insufficient.connect(self.handle_insufficient_budget)
        self.print_validation_service.print_budget_updated.connect(self.update_budget_display)
    ...
    def set_print_job(self, black_white_pages: int, color_pages: int):
        ...
        if self.validation_result.get("success"):
            ...
            self.show_error_message(self.validation_result.get("error", "שגיאה לא ידועה"))
    ...
    def update_validation_display(self):
        ...
        if can_print:
            ...
            self.status_label.setStyleSheet(f"color: {Colors.SUCCESS}; font-weight: bold; margin-left: 10px;")
            self.print_button.setEnabled(True)
        else:
            self.status_label.setText("❌ יתרה לא מספקת")
            self.status_label.setStyleSheet(f"color: {Colors.ERROR}; font-weight: bold; margin-left: 10px;")
            self.print_button.setEnabled(False)
    ...
    def approve_print(self):
        ...
        logger.info(f"Print job approved: {self.black_white_pages} B&W, {self.color_pages} color")
