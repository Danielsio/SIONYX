...
class PrintPricingDialog(QDialog):
    ...
    def __init__(self, organization_metadata_service: OrganizationMetadataService, org_id: str, parent=None):
    ...
    def init_ui(self):
        ...
        bw_label.setStyleSheet(f"font-weight: bold; color: {Colors.TEXT_PRIMARY}; min-width: 150px;")
        
        self.bw_price_input = QLineEdit()
        self.bw_price_input.setPlaceholderText("1.0")
        self.bw_price_input.setStyleSheet(f"""
            QLineEdit {{
                padding: 8px;
                border: 1px solid {Colors.BORDER};
                border-radius: 4px;
        ...
        color_label.setStyleSheet(f"font-weight: bold; color: {Colors.TEXT_PRIMARY}; min-width: 150px;")
        
        self.color_price_input = QLineEdit()
        self.color_price_input.setPlaceholderText("3.0")
        self.color_price_input.setStyleSheet(f"""
            QLineEdit {{
                padding: 8px;
                border: 1px solid {Colors.BORDER};
                border-radius: 4px;
        ...
        currency_note.setStyleSheet(f"color: {Colors.TEXT_SECONDARY}; font-style: italic; margin-top: 10px;")
    ...
    def load_current_pricing(self):
        ...
        try:
            ...
            if result.get("success"):
                ...
                self.bw_price_input.setText(str(pricing.get("black_and_white_price", 1.0)))
    ...
    def save_pricing(self):
        ...
        try:
            ...
            if result.get("success"):
                ...
                QMessageBox.critical(self, "שגיאה", f"נכשל לשמור מחירי הדפסה: {result.get('error')}")
