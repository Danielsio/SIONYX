...
class BaseService(ABC):
    ...
    def create_error_response(self, error: str, error_code: str = None) -> Dict[str, Any]:
    ...
    def handle_firebase_error(self, error: Exception, operation: str) -> Dict[str, Any]:
        ...
        if "permission-denied" in error_msg.lower():
            return self.create_error_response("אין הרשאה לבצע פעולה זו", "PERMISSION_DENIED")
        elif "network" in error_msg.lower() or "timeout" in error_msg.lower():
            return self.create_error_response("שגיאת רשת - נסה שוב מאוחר יותר", "NETWORK_ERROR")
        elif "not-found" in error_msg.lower():
            return self.create_error_response("הנתונים לא נמצאו", "NOT_FOUND")
        else:
            return self.create_error_response(f"שגיאה ב{operation}: {error_msg}", "UNKNOWN_ERROR")
    ...
    def safe_int(self, value: Any, default: int = 0) -> int:
        ...
        try:
            ...
            self.logger.warning(f"Could not convert '{value}' to int, using default {default}")
            return default
    ...
    def require_authentication(self) -> Optional[Dict[str, Any]]:
        ...
        if not self.is_authenticated():
            return self.create_error_response("נדרשת התחברות", "AUTHENTICATION_REQUIRED")
        return None
...
class DatabaseService(BaseService):
    ...
    def get_all_documents(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.handle_firebase_error(e, f"get all documents from {self.collection_name}")
    
    def create_document(self, data: Dict[str, Any], doc_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.handle_firebase_error(e, f"create document in {self.collection_name}")
