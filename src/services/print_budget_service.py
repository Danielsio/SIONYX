...
class PrintBudgetService:
    ...
    def get_organization_print_pricing(self, org_id: str) -> Dict:
        ...
        try:
            ...
            if not result.get("success"):
                logger.error(f"Failed to get organization metadata: {result.get('error')}")
                return {
                    "success": False,
            ...
            pricing = {
                "black_and_white_price": metadata.get("blackAndWhitePrice", 1.0),  # Default 1 NIS
    ...
    def set_organization_print_pricing(self, org_id: str, black_white_price: float, color_price: float) -> Dict:
        """
    ...
    def get_user_print_budget(self, user_id: str) -> Dict:
        ...
        try:
            ...
            budget = user_data.get("remainingPrints", 0.0)  # Budget in NIS (renamed from count)
    ...
    def calculate_print_cost(self, org_id: str, black_white_pages: int, color_pages: int) -> Dict:
        """
    ...
    def validate_print_budget(self, user_id: str, org_id: str, black_white_pages: int, color_pages: int) -> Dict:
        """
        ...
        try:
            ...
            cost_result = self.calculate_print_cost(org_id, black_white_pages, color_pages)
            ...
            remaining_after_print = user_budget - total_cost if has_sufficient_budget else user_budget
            ...
            return {
                ...
                "insufficient_amount": total_cost - user_budget if not has_sufficient_budget else 0
            }
    ...
    def deduct_print_budget(self, user_id: str, org_id: str, black_white_pages: int, color_pages: int) -> Dict:
        """
        ...
        try:
            ...
            validation_result = self.validate_print_budget(user_id, org_id, black_white_pages, color_pages)
            ...
            result = self.firebase.db_update(f"users/{user_id}", {
            ...
            if not result.get("success"):
                logger.error(f"Failed to update user print budget: {result.get('error')}")
                return {
                    "success": False,
                    "error": f"Failed to update user print budget: {result.get('error')}"
                }
            
            logger.info(f"Print budget deducted for user {user_id}: {print_cost} NIS, remaining: {new_budget} NIS")
    ...
    def add_print_budget(self, user_id: str, amount: float, reason: str = "purchase") -> Dict:
        """
        ...
        try:
            ...
            logger.info(f"Print budget added for user {user_id}: +{amount} NIS, new total: {new_budget} NIS")
            ...
            return {
                "success": False,
                "error": f"Error adding print budget: {str(e)}"
            }
