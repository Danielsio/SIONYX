...
class PrintValidationService(QObject):
    ...
    # Signals
    print_budget_insufficient = pyqtSignal(float, float)  # current_budget, required_amount
    ...
    def validate_print_job(self, black_white_pages: int, color_pages: int) -> Dict:
        ...
        """
        try:
            logger.info(f"Validating print job: {black_white_pages} B&W, {color_pages} color pages")
            ...
            if not validation_result.get("success"):
                logger.error(f"Print budget validation failed: {validation_result.get('error')}")
            ...
            if not can_print:
                ...
                logger.warning(f"Insufficient print budget: {user_budget} NIS, need {print_cost} NIS")
            ...
            return {
                ...
                "user_budget": user_budget,
                "print_cost": print_cost,
                "remaining_after_print": validation_result.get("remaining_after_print", 0),
            ...
            return {
                "success": False,
                "error": f"Error validating print job: {str(e)}"
            }
    ...
    def process_successful_print(self, black_white_pages: int, color_pages: int) -> Dict:
        ...
        """
        try:
            logger.info(f"Processing successful print: {black_white_pages} B&W, {color_pages} color pages")
            ...
            if not deduction_result.get("success"):
                logger.error(f"Failed to deduct print budget: {deduction_result.get('error')}")
            ...
            logger.info(f"Print budget deducted: {deducted_amount} NIS, remaining: {new_budget} NIS")
    ...
    def simulate_print_cost(self, black_white_pages: int, color_pages: int) -> Dict:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Error simulating print cost: {str(e)}"
            }
