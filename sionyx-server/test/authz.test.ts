...
/** POST as `uid` (a real signed ID token, verified against the mocked JWKS). */
async function post(path: string, body: unknown, uid: string | null): Promise<Response> {
...
describe('usage deductions are self-only', () => {
  it('deducts time for the token owner', async () => {
    ...
    const res = await post('/usage/deduct-time', { orgId: 'org1', userId: 'u1', seconds: 60 }, 'u1');
    expect(res.status).toBe(200);
    expect(await res.json()).toMatchObject({ success: true, remainingTime: 60 });
  });

  it("rejects deducting another user's time", async () => {
    const res = await post('/usage/deduct-time', { orgId: 'org1', userId: 'u1', seconds: 60 }, 'attacker');
  ...
  it("rejects deducting another user's print balance", async () => {
    const res = await post('/usage/deduct-print', { orgId: 'org1', userId: 'u1', amount: 1 }, 'attacker');
    expect(res.status).toBe(401);
  });

  it('rejects a deduction without a bearer token', async () => {
    const res = await post('/usage/deduct-time', { orgId: 'org1', userId: 'u1', seconds: 60 }, null);
    expect(res.status).toBe(401);
  });
});
...
describe('/admin/adjust-balance requires an org admin', () => {
  it('rejects a non-admin caller', async () => {
    ...
    const res = await post('/admin/adjust-balance', { orgId: 'org1', userId: 'u1', addSeconds: 9999 }, 'u2');
  ...
  it('adjusts balances for an admin caller', async () => {
    ...
    const res = await post('/admin/adjust-balance', { orgId: 'org1', userId: 'u1', addSeconds: 60 }, 'boss');
    expect(res.status).toBe(200);
